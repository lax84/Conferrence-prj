...
export class EventService {
  constructor(private http:HttpClient){
    ...
    getEvents() {
        return this.http.get("http://localhost:8080/root/allevents.json");
    }
    getEvent(eventId:number){
        //return EVENTS.find((event)=> event.id===eventId); //=> this is called as LAMBDA expression in TypeScript
        const headers=new HttpHeaders({'Authorization':'Basic '+btoa("Lakshman:Addhu")});
        return this.http.get("http://localhost:8080/root/eventdetails/"+eventId+".json",{headers});
    }
}
const EVENTS = [];
// const EVENTS = [
